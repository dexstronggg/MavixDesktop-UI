...
(`run_tests.sh`, отчёт в `htmlcov/`). Текущий результат: **244 теста проходят,
покрытие кода 53%**. Доля непокрытого кода приходится на графический интерфейс,
который проверяется отдельно; бизнес-логика (сессия, телеметрия, карта) покрыта
значительно плотнее.
