...
3. Передача пакетов FC и обработка сообщений `config-channel`
   ...
   - `focus`: `{'type': 'focus', 'device_index': N}` — активная камера
     (борт снижает частоту остальных до `IDLE_FPS`, не обрывая их поток).
...
`encoder.build_rc_frame(throttle, roll, pitch, yaw, armed, aux=None)` —
преобразует положения стиков (в диапазоне [-1; 1]) в кадр CRSF
(тип 0x16, 16 каналов, порядок TAER). `aux` — значения CH6…CH16;
короткий список дополняется центром, длинный обрезается в
`CRSF.rc_frame`, поэтому кадр всегда шестнадцатиканальный.

**Раскладка каналов** (`joystick/channels.py`):

| Канал | Источник |
|---|---|
| CH1–CH4 | стики, порядок TAER |
| CH5 | ARM |
| CH6 | сброс груза; центр, если кнопка не назначена |
| CH7…CH16 | свободные органы: кнопки, затем незанятые оси, затем крестовина |

`auto_bindings()` строит раскладку детерминированно и исключает то, что
уже занято стиками, ARM и сбросом. CH6 закреплён за сбросом всегда, даже
когда кнопка не назначена: иначе при её настройке раскладка сдвинулась бы
и правила `aux` в полётнике указывали бы не на те каналы.

Кнопки работают как фиксаторы — состояние переключается по фронту
нажатия (`JoystickInput._toggle_button`), потому что полётнику нужно
устойчивое положение канала, а кнопка геймпада отдаёт сигнал только пока
нажата. Оси передаются пропорционально, крестовина — тремя положениями.

При сбое чтения джойстика `get_aux_channels()` возвращает **прошлые**
значения, а аварийный failsafe в `FlightWindow` шлёт последний известный
`aux`. Обнуление в момент потери джойстика дёрнуло бы сброс груза.

`JoystickInput.is_released()` повторяет защёлку `is_armed()`: пока не
зафиксировано положение «не сброшено», метод возвращает False. Без неё
вход в полёт с тумблером, забытым во включённом положении, раскрыл бы
замок в первом же кадре. Состояние фиксатора живёт в `self._release` и
защёлкой не затирается — иначе кнопочный сброс перестал бы переключаться.

`FlightWindow` показывает состояние груза над надписью `ARM`/`DISARM`
(`ГРУЗ НА БОРТУ` / `ГРУЗ СБРОШЕН`) и скрывает её, когда
`has_release()` вернул False.

Каналы CH6 и выше уходят **только по CRSF**, то есть работают лишь с
Betaflight и iNav. `MavlinkEncoder` передаёт команды (`arm_disarm`,
`set_mode`, `failsafe_rtl`), а не RC-каналы, поэтому на PX4 и ArduPilot
тумблеры и сброс груза не работают вовсе.
