"""UI-facing adapter around mavixdesktop.coordinator.SessionCoordinator.

The Coordinator runs an asyncio event loop in a background thread.
This adapter:
  - boots the loop on construction
  - logs the user in (or restores from keyring) and starts the coordinator
  - forwards coordinator events into the Qt Bridge signals
  - exposes synchronous UI methods that schedule coroutines onto the
    background loop
"""
...
class ConnectionManager:
    ...
    def login(self, email: str, password: str) -> None:
        """Run login in the background loop; UI doesn't block."""
    ...
    def resume(self) -> bool:
        """Try to restart with the stored refresh token. Returns True if found."""
    ...
    def _ensure_loop_started(self) -> None:
        ...
        self._thread.start()
        logger.info('[connection] event loop thread started')
    ...
    async def _login_and_run(self, email: str, password: str) -> None:
        ...
        try:
            ...
            logger.error('[connection] login failed: %s', exc)
            await self._api.close()
            self._api = None
            self._emit_login_failed(str(exc))
            return
        except Exception as exc:
            logger.exception('[connection] login crashed: %s', exc)
    ...
    async def _refresh_and_run(self, email: str, refresh_token: str) -> None:
        ...
        try:
            ...
            logger.warning('[connection] refresh failed (%s); clearing stored token', exc)
            token_store.clear()
            await self._api.close()
            self._api = None
    ...
    def _emit_login_succeeded(self) -> None:
        try:
            ...
            logger.warning('[connection] bridge emit error: %s', exc)
    ...
    def _on_session_ended(self) -> None:
        if self._reset_callback is not None:
            try:
                ...
                logger.warning('[connection] reset callback error: %s', exc)
