...
Собирает VideoPanel (видео + оверлей) и SettingsBar (нижняя панель)
...
class DroneViewPage(QWidget):
    ...
    def __init__(self, on_back: Callable, on_prev: Callable, on_next: Callable,
                 on_save: Callable, on_joystick_cfg: Callable, on_takeoff: Callable,
    ...
    def update_ping(self, rtt_ms: float):
        """Обновить показание peer-to-peer пинга."""
        self._video_panel.update_ping_overlay(rtt_ms)

    def update_battery(self, percent: int, voltage: float):
        """Обновить overlay батареи (CRSF BATTERY_SENSOR от борта)."""
        self._video_panel.update_battery_overlay(percent, voltage)

    def update_camera_settings(self, camera: dict):
        """Обновить дропдауны настроек камеры."""
        self._settings_bar.update_camera(camera)

    def get_selected_params(self):
        """Вернуть (param_index, bitrate_kbs) по текущему выбору."""
        return self._settings_bar.get_selected_params()

    def set_calibration_visible(self, visible: bool):
        """Показать/скрыть оверлей «Идёт калибровка камеры…»."""
        self._video_panel.set_calibration_visible(visible)

    def show_error_banner(self, text: str) -> None:
        """Показать красный баннер по центру (для ошибок установки сессии)."""
